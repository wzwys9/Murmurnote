...
class AppPreferences internal constructor(
    ...
    @Inject
    constructor(@ApplicationContext applicationContext: Context) : this(
    ...
    private fun activeLlmApiKey(prefs: Preferences): String {
    ...
    val llmApiKey: Flow<String> = dataStore.data.map(::activeLlmApiKey)

    val llmProvider: Flow<String> = dataStore.data.map {
        it[Keys.LLM_PROVIDER] ?: LlmProvider.DEEPSEEK.name
    }

    val llmModel: Flow<String> = dataStore.data.map {
        it[Keys.OLLAMA_MODEL].orEmpty()
    }

    val reasoningEffort: Flow<String> = dataStore.data.map {
        it[Keys.REASONING_EFFORT] ?: "high"
    }

    val glmBaseUrl: Flow<String> = dataStore.data.map {
        it[Keys.GLM_BASE_URL]?.takeIf { it.isNotBlank() } ?: "https://open.bigmodel.cn/api/paas/v4/"
    }

    val llmBaseUrl: Flow<String> = dataStore.data.map {
    ...
    val systemPromptOverride: Flow<String?> = dataStore.data.map {
        it[Keys.SYSTEM_PROMPT_OVERRIDE]
    }

    val userPromptOverride: Flow<String?> = dataStore.data.map {
        it[Keys.USER_PROMPT_OVERRIDE]
    }

    val debugForceNetworkFail: Flow<Boolean> = dataStore.data.map {
        it[Keys.DEBUG_FORCE_NETWORK_FAIL] ?: false
    }

    val debugSimulateDelayMs: Flow<Long> = dataStore.data.map {
        it[Keys.DEBUG_SIMULATE_DELAY_MS]?.toLongOrNull() ?: 0L
    }

    /** 新安装默认本地 SenseVoice；已完成 onboarding 的旧安装保持云端默认。 */
    val asrEngineType: Flow<String> = dataStore.data.map { prefs ->
    ...
    val asrDownloadMirrorIndex: Flow<Int> = dataStore.data.map {
        it[Keys.ASR_DOWNLOAD_MIRROR_INDEX]?.toIntOrNull() ?: 0
    }

    /** 当前选中的本地 ASR 模型。默认 SenseVoiceSmall，旧安装升级后行为不变。 */
    val asrLocalModelId: Flow<String> = dataStore.data.map {
    ...
    val asrLanguageMode: Flow<AsrLanguageMode> = dataStore.data.map {
        AsrLanguageMode.parse(it[Keys.ASR_LANGUAGE_MODE])
    }

    val asrManualLanguage: Flow<String> = dataStore.data.map {
        AsrLanguagePolicy.normalizeManualLanguage(it[Keys.ASR_MANUAL_LANGUAGE].orEmpty())
            ?: AsrLanguagePolicy.AUTO
    }

    val asrSenseVoiceUseItn: Flow<Boolean> = dataStore.data.map {
        it[Keys.ASR_SENSE_VOICE_USE_ITN] ?: true
    }

    val realtimePerformanceMode: Flow<String> = dataStore.data.map {
        it[Keys.REALTIME_PERFORMANCE_MODE] ?: "BALANCED"
    }

    val lowBatteryProtection: Flow<Boolean> = dataStore.data.map {
        it[Keys.LOW_BATTERY_PROTECTION] ?: true
    }

    val aiExtractionEnabled: Flow<Boolean> = dataStore.data.map { prefs ->
    ...
    suspend fun setGlmApiKey(key: String) = dataStore.edit { it[Keys.GLM_API_KEY] = key.trim() }
    suspend fun setLlmApiKey(key: String) {
        val provider = LlmProvider.parse(dataStore.data.first()[Keys.LLM_PROVIDER])
        setLlmApiKey(provider, key)
    }
    suspend fun setLlmApiKey(provider: LlmProvider, key: String) {
        val normalizedKey = key.trim()
        dataStore.edit { prefs ->
            val activeProvider = LlmProvider.parse(prefs[Keys.LLM_PROVIDER])
    ...
    suspend fun setLlmProvider(provider: LlmProvider) = dataStore.edit {
    ...
    suspend fun setReasoningEffort(effort: String) = dataStore.edit { it[Keys.REASONING_EFFORT] = effort }
    suspend fun setGlmBaseUrl(url: String) = dataStore.edit { it[Keys.GLM_BASE_URL] = url.trim() }
    suspend fun setLlmBaseUrl(url: String) = dataStore.edit { it[Keys.OLLAMA_BASE_URL] = url.trim() }
    suspend fun completeOnboarding() = dataStore.edit { prefs ->
    ...
    suspend fun setUserPromptOverride(p: String?) = dataStore.edit {
        if (p == null) it.remove(Keys.USER_PROMPT_OVERRIDE) else it[Keys.USER_PROMPT_OVERRIDE] = p
    }

    suspend fun setDebugForceNetworkFail(v: Boolean) = dataStore.edit { it[Keys.DEBUG_FORCE_NETWORK_FAIL] = v }
    suspend fun setDebugSimulateDelayMs(ms: Long) = dataStore.edit { it[Keys.DEBUG_SIMULATE_DELAY_MS] = ms.toString() }
    suspend fun setAsrEngineType(t: String) = dataStore.edit {
        it[Keys.ASR_ENGINE_TYPE] = AsrEngineType.parse(t).name
    }
    suspend fun setAsrLocalModelId(id: String) = dataStore.edit { it[Keys.ASR_LOCAL_MODEL_ID] = id }
    suspend fun setAsrDownloadMirrorIndex(i: Int) = dataStore.edit { it[Keys.ASR_DOWNLOAD_MIRROR_INDEX] = i.toString() }
    suspend fun setAsrBundledInstalled(v: Boolean) = dataStore.edit { it[Keys.ASR_BUNDLED_INSTALLED] = v }
    suspend fun setAsrLocalConcurrency(v: Int) = dataStore.edit { it[Keys.ASR_LOCAL_CONCURRENCY] = v.coerceIn(1, 3) }
    suspend fun setAsrLanguageMode(mode: AsrLanguageMode) = dataStore.edit {
    ...
    suspend fun setAsrManualLanguage(language: String): Boolean {
        ...
        dataStore.edit { it[Keys.ASR_MANUAL_LANGUAGE] = normalized }
        return true
    }
    suspend fun setAsrSenseVoiceUseItn(enabled: Boolean) = dataStore.edit {
        it[Keys.ASR_SENSE_VOICE_USE_ITN] = enabled
    }
    suspend fun setRealtimePerformanceMode(v: String) = dataStore.edit { it[Keys.REALTIME_PERFORMANCE_MODE] = v }
